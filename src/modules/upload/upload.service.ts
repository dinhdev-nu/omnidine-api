...
import { BadRequestException, TooManyRequestException } from 'src/common/exceptions';
...
export class UploadService {
    ...
    async upload(file: Express.Multer.File, userId: string): Promise<UploadResponseDto> {

        // Rate limit 
        const uploadKey = `${RATE_LIMIT_UPLOAD_KEY_PREFIX}${userId}`;
        const currentUploads = await this.redis.incr(uploadKey);
        if (currentUploads === 1) await this.redis.expire(uploadKey, RATE_LIMIT_UPLOAD_TTL_SECONDS);
        if (currentUploads > RATE_LIMIT_MAX_UPLOADS) {
            throw new TooManyRequestException(ERROR_CODE.TOO_MANY_REQUESTS, 'Upload rate limit exceeded. Please try again later.');
        }
       ...
       try {
            ...
            throw new BadRequestException(ERROR_CODE.UPLOAD_ERROR , 'Failed to upload file');
       } finally {
    ...
    async multipleUpload(files: Express.Multer.File[], userId: string): Promise<UploadResponseDto[]> {
        ...
        if (currentMultiUploads > RATE_LIMIT_MAX_UPLOADS) {
            throw new TooManyRequestException(ERROR_CODE.TOO_MANY_REQUESTS, 'Multiple upload rate limit exceeded. Please try again later.');
        }

        const uploadPromises = files.map(file => this.upload(file, userId));
        return Promise.allSettled(uploadPromises).then(results => {
            const successfulUploads = results.filter(result => result.status === 'fulfilled') as PromiseFulfilledResult<UploadResponseDto>[];
            const failedUploads = results.filter(result => result.status === 'rejected') as PromiseRejectedResult[];    
            if (failedUploads.length > 0) {
                throw new BadRequestException(ERROR_CODE.UPLOAD_ERROR , `${failedUploads.length} out of ${files.length} files failed to upload`);
            }
            return successfulUploads.map(result => result.value);
        });
    }

    async replaceUpload(oldImgUrl: string, newFile: Express.Multer.File, userId: string): Promise<UploadResponseDto> {
        ...
        try {
            ...
            throw new BadRequestException(ERROR_CODE.UPLOAD_ERROR , 'Failed to replace file');  
        }
    }
    ...
    async delete(imgUrl: string): Promise<void> {
        ...
        try {
            ...
            throw new BadRequestException(ERROR_CODE.DELETE_ERROR , 'Failed to delete file');
        }
    }
